...
import NumberInput from './containers/number-input.jsx';
import LayerListContainer from './containers/layer-list.jsx';
...
class Playground extends React.Component {
    ...
    editLayer(id, data) {
        this.setState({ layers: { ...this.state.layers, [id]: data } });
        console.log("layerediteded")
    }

    createNewLayer() {
        this.id++;
        this.editLayer(this.id, {
            name: 'meow',
            rotationCenterX: 20,
            rotationCenterY: 400,
            imageFormat: 'svg', // 'svg', 'png', or 'jpg'
        ...
        this.setActiveLayer(this.id);
    }

    setActiveLayer(id) {
        console.log("activelayerchanged")
        if (!typeof this.state.activeLayer === null) {
            this.editLayer(activeLayer,
                {
        ...
        let activeLayerData = this.getLayerData(id);
        this.setState({
            name: activeLayerData.name
        })
        this.handleUpdateImage(activeLayerData.imageFormat,
                        activeLayerData.image,
            activeLayerData.rotationCenterX,
            activeLayerData.rotationCenterY,
        );
                ...
                <div className={styles.paintWrapper}>
                    <div className={styles.layerList}>
                        <LayerListContainer
                            ...
                            getLayerData={this.getLayerData}
                            allLayerData={this.state.layers}
...
{/*                 <button className={styles.playgroundButton} onClick={this.uploadImage}>Upload</button>
                ...
                <NumberInput id={"x"} placeholder={480} updateTo={this.updateStageSize} />
                <NumberInput id={"y"} placeholder={360} updateTo={this.updateStageSize} /> */}
            </div>
        );
    }
